...
4. Handle external checks (non-GitHub Actions).
   - If `detailsUrl` is not a GitHub Actions run (e.g., Hamatoyshi (Jenkins), CircleCI), fetch output via the Checks API:
     ```bash
     gh api repos/{owner}/{repo}/commits/$(git rev-parse HEAD)/check-runs \
       --jq '.check_runs[] | select(.conclusion == "failure") | {name: .name, output: .output}' 2>&1 | cat
     ```
   - This returns the check name and output summary/text for failed external checks.
   - If the output field is empty or null, report the `detailsUrl` and note logs must be viewed externally.
   - Do not attempt to fetch full logs beyond what the Checks API provides.
